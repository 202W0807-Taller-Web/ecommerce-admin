...
import { getAlmacen } from "../../modules/inventario-envios/local/api/almacenes";
...
export default function AlmacenesDetailsPage() {
  ...
  const {
    ...
    queryKey: ["almacen", id],
    queryFn: () => getAlmacen(Number(id)),
    enabled: !!id,
  });

  // TIENDAS DEL ALMACÉN
  const [searchFiltersTiendas, setSearchFiltersTiendas] = useState({
    nombre: "",
    departamento: "",
    provincia: "",
    distrito: "",
  ...
  const [filtersTiendas, setFiltersTiendas] = useState({
    nombre: "",
    departamento: "",
    provincia: "",
    distrito: "",
  ...
  const { page: pageTiendas, setPage: setPageTiendas } = usePagination();
  const ubicacionRef = useRef<{ reset: () => void }>(null);

  const {
    data,
    isPending: tiendasPending,
    isError: tiendasError,
  } = useQuery<Root<Tienda>>({
    queryKey: [
      "tiendas",
      {
        ...filtersTiendas,
  ...
  const tiendasData = data?.data ?? [];
  const paginationTiendas = data?.pagination;

  // Manejo de filtros
  const handleFilterSubmit = () => {
    setFiltersTiendas(searchFiltersTiendas);
    setPageTiendas(1);
  };

  const handleResetFilters = () => {
    ubicacionRef.current?.reset();
    setSearchFiltersTiendas({
      nombre: "",
      departamento: "",
      provincia: "",
      distrito: "",
  ...
  return (
    <div className="p-4 w-full max-w-full overflow-x-auto">
      ...
      {isPending ? (
        ...
        <p>Error al cargar los datos del almacén.</p>
      ) : almacen ? (
        <div className="flex flex-col sm:flex-row gap-6 mb-6 items-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
            <div className="flex flex-col gap-2">
              <div>
                <b>Dirección:</b> {almacen.direccion}
              </div>
              <div>
                <b>Estado:</b>{" "}
                <span
                  className={`${almacen.estado.toLowerCase() === "activo" ? "text-green-700" : "text-red-700"}`}
                >
                  {almacen.estado}
                </span>
              </div>
              <div>
                <b>Distrito:</b> {almacen.distrito}
              </div>
            </div>
            <div className="flex flex-col gap-2">
              <div>
                <b>Provincia:</b> {almacen.provincia}
              </div>
              <div>
                <b>Departamento:</b> {almacen.departamento}
              </div>
            </div>
          </div>
        </div>
                ...
                <tr key={p.id} className={idx % 2 ? "bg-gray-50" : "bg-white"}>
                  ...
                  <TableCell>
                    <StatusBadge
                      label={p.estadoStk}
                      variant={
                        p.estadoStk === "Disponible" ? "success" : "danger"
                      }
                    />
                  </TableCell>
                  <ActionMenuCell
                    buttons={[
                      {
                        ...
                        onClick: () =>
                          console.log(`Editar producto: ${p.producto}`),
                      },
                      {
                        label: "Eliminar",
                        icon: <Trash2 className="w-4 h-4 text-red-600" />,
                        onClick: () =>
                          console.log(`Eliminar producto: ${p.producto}`),
                      },
                      {
                        label: "Asignar",
                        icon: (
                        ...
                        onClick: () =>
                          console.log(`Asignar producto: ${p.producto}`),
                      },
                    ]}
                  />
                </tr>
      ...
      <div className="mt-10">
        ...
        >
          <Input
            ...
            value={searchFiltersTiendas.nombre}
            onChange={e =>
              setSearchFiltersTiendas(prev => ({
                ...prev,
          ...
          <SelectsUbigeo
            ref={ubicacionRef}
            onChange={values =>
              setSearchFiltersTiendas(prev => ({ ...prev, ...values }))
            }
          />
        </FilterForm>
        {/* Acciones tiendas */}
        ...
        <FooterTable
          page={pageTiendas}
          limit={limit}
          pagination={paginationTiendas}
          handlePageChange={setPageTiendas}
        />
      </div>
    </div>
  );
